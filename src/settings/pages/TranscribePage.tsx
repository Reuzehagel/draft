    ...
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        description={isOnline
          ? `Using ${STT_PROVIDER_LABELS[sttProvider] ?? sttProvider}`
          : "Transcribe audio files using Whisper"}
      />

      <SettingsCard title="Audio File">
        ...
        {ft.isDecoding && <Progress value={ft.decodeProgress} className="h-1.5" />}
        {ft.isTranscribing && (
          ft.transcriptionProgress > 0 ? (
            <Progress value={ft.transcriptionProgress} className="h-1.5" />
          ) : (
            <div className="w-full overflow-hidden rounded-full h-1.5 bg-muted">
